# Phage ATAC Preprocessing

A script to process fastqs from Phage ATAC for downstream processing with kite (`kallisto | bustools`). 
...
```
python phage_to_kite-R3.py -f test/data1 -s test_EF_hPBMC -o one_one
```
...
```
python phage_to_kite-R3.py -f test/data1,test/data2 -s test1 -o one_many
```
...
```
python phage_to_kite-R3.py -f test/data1,test/data2 -s test_EF_hPBMC,test2_EF_hPBMC -o many_many
```
...
```
python phage_to_kite-R3.py -f test/data1,test/data2 -s test_EF_hPBMC,test2_EF_hPBMC -o test/many_many
```
...
```
python phage_to_kite-R3.py --help
```

yields

```
Usage: phage_to_kite-R3.py [options] [inputs] Script to reformat raw sequencing 
data from CellRanger-ATAC demultiplexing for PHAGE ATAC to a format 
...
Options:
  ...
  -x R3_BARCODE_START_INDEX, --R3-barcode-start-index=R3_BARCODE_START_INDEX
                        The starting position of the CDR3 sequence in R3 based
