...
opts.add_option("--cores", '-c', default = 1, help="Number of cores for parallel processing. Default = 4.")
...
opts.add_option("--R3-barcode-start-index", '-x', default = 21, help="The starting position of the CDR3 sequence in R3 based on the sequencing configuration")
...
start_idx = int(options.R3_barcode_start_index)

# Establish a fake UMI
fake_UMI = "A"*10
...
def phage_to_kite_r3(trio):
	...
	new_sequence1 = sequence2 + fake_UMI
	new_sequence2 = sequence3[start_idx:]
	print(new_sequence1)
	print(new_sequence2)
	
	new_quality1 = quality2 + quality2[0:10] # second quality is a dummy
	new_quality2 = quality3[start_idx:]
...
if __name__ == "__main__":
	print("\nPhage-to-kite for R3, version 1\n")
		...
		with gzip.open(outfq2file, "wt") as out_f2:
			for R1file in R1s_for_analysis:
				...
				for i, batch_R1 in enumerate(it1):
					...
					pm = pool.map(phage_to_kite_r3, zip(batch_R1, batch_R2, batch_R3))
					pool.close()
